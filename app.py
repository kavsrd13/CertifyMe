...
from agents.client import memory_exporter
...
app_tab, obs_tab = st.tabs(["🎯 Learning Pipeline", "📊 Observability Dashboard"])
...
with obs_tab:
    st.markdown("""
    <div style='margin-bottom:1.4rem'>
      <h2 style='font-size:1.55rem;font-weight:700;color:#F2F4F7;margin:0'>Agent Observability</h2>
      <p style='color:#8896AB;margin:.35rem 0 0;font-size:.93rem'>
        Live tracing of Microsoft Agent Framework components via OpenTelemetry.<br>
        <small>Powered by `InMemorySpanExporter`.</small>
      </p>
    </div>""", unsafe_allow_html=True)

    if st.button("Refresh Traces", icon="🔄"):
        st.rerun()

    spans = memory_exporter.get_finished_spans()
    
    if not spans:
        st.info("No agents have been invoked yet. Run through the pipeline first!")
    else:
        st.markdown("<hr class='div-line'>", unsafe_allow_html=True)
        ...
        c1, c2, c3 = st.columns(3)
        c1.metric("Total Agent Invocations", len(agent_spans))
        c2.metric("Total Chat Completions", len(chat_spans))
        c3.metric("Total Tool Executions", len(tool_spans))
        ...
        for span in reversed(agent_spans):
            ...
            duration = (span.end_time - span.start_time) / 1e9  # nanoseconds to seconds
            
            with st.expander(f"🤖 {agent_id} — {duration:.2f}s"):
                st.markdown(f"**Instructions:**\n`{attrs.get('gen_ai.request.instructions', 'N/A')}`")
                
                # Find matching chat span for token usage
                for c_span in reversed(chat_spans):
                    if c_span.parent and c_span.parent.span_id == span.get_span_context().span_id:
                        c_attrs = c_span.attributes or {}
                        in_toks = c_attrs.get("gen_ai.usage.input_tokens", 0)
                        out_toks = c_attrs.get("gen_ai.usage.output_tokens", 0)
                        
                        st.markdown(f"""
                        <div style='background:rgba(255,255,255,0.03);padding:1rem;border-radius:10px;margin-top:1rem;'>
                            <strong>Token Usage:</strong> {in_toks} <span style='color:#8896AB'>input</span> &nbsp;|&nbsp; 
                            {out_toks} <span style='color:#8896AB'>output</span> &nbsp;|&nbsp; 
                            <strong>{in_toks + out_toks}</strong> <span style='color:#A5B4FC'>total</span>
                        </div>
                        """, unsafe_allow_html=True)
                        break
